# AnswerBot-Screen — AI 自动答题机器人（读屏版 v2.1）
...
### 6.2 变化检测 + 题目区域定位（v2.1）

- 缩略图 pHash 判变化，连续 `n_stable` 帧稳定才识别（防动画半帧）；
...
| `t_change` | 12 | 变化判定阈值；画面抖动频繁可调大 |
| `t_stable` / `n_stable` | 4 / 6 | 稳定判定；翻页动画长可调大 `n_stable` |
...
- v2.1（实机反馈修订）：窗口抓取 → 全屏抓取；新增 ROI 差分定位、30s 饱和式兜底、
