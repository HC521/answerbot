# AnswerBot-Screen — AI 自动答题机器人（读屏版 v2.3）
...
5. 漏判时点悬浮窗右上角「**保底**」按钮（v2.3）立即识别；也可按**绑定的手动触发键**（可选，见 6.3）。
...
- v2.3（实机反馈修订）：悬浮窗右上角新增「保底」按钮（点击立即识别）；
  变化检测与 ROI 定位完全排除悬浮窗区域（其显示答案/拖拽不再误触发，
  全图识别时悬浮窗区域自动涂白防模型误读）；
